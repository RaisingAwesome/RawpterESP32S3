...
static float i_limit = 25.0f;   // Integrator saturation level
static float i_limit_rate = 25.0f;   // Integrator saturation level

static float Kp_roll_angle = 0.0f;  // Roll P-gain
static float Ki_roll_angle = 0.0f;  // Roll I-gain
static float Kd_roll_angle = 0.0f;  // Roll D-gain

static float Kp_pitch_angle = 0.0f;  // Pitch P-gain
static float Ki_pitch_angle = 0.0f;  // Pitch I-gain
static float Kd_pitch_angle = 0.0f;  // Pitch D-gain

static float Kp_roll_rate = 0.0f;  // Roll P-gain
static float Ki_roll_rate = 0.0f;  // Roll I-gain
static float Kd_roll_rate = 0.0f;  // Roll D-gain

static float Kp_pitch_rate = 0.0f;  // Pitch P-gain
static float Ki_pitch_rate = 0.0f;  // Pitch I-gain
static float Kd_pitch_rate = 0.0f;  // Pitch D-gain

static float Kp_yaw_rate = 0.0f;  // Yaw P-gain default 30
static float Ki_yaw_rate = 0.0f;   // Yaw I-gain default 5
static float Kd_yaw_rate = 0.0f;   // Yaw D-gain default .015 (be careful when increasing too high, motors will begin to overheat!)

struct Limits { // Rate Limits for S-Curve
    float maxRate;   // deg/s
    float maxAccel;  // deg/s^2
    float maxJerk;   // deg/s^3
};

// Instantiate per-axis limits (conservative values for 10" props)
Limits rollLimits  = {220.0f, 1400.0f, 18000.0f}; // maxRate, maxAccel, maxJerk
Limits pitchLimits = {220.0f, 1400.0f, 18000.0f};
Limits yawLimits   = {180.0f, 1400.0f, 18000.0f};
...
static BMP581 pressureSensor;
static uint8_t i2cAddress = 0x46; // 0x46 is the Pressure Sensor 0x42 is the GPS
static volatile float altitude = 0.0f; // just a default until it starts to loop.
static volatile float startAltitude = 0.0f; // used to set target altitude as a differential from ground.
static volatile float groundDistance = 0.0f; // used to set target altitude as a differential from ground.
static float highestAltitude = 0.0f;
static const float ceiling = 12.0f;
static volatile float rateFPS = 0.0f; // used to ensure it doesn't descend too quickly and 
static volatile float rateFPSWorking = 0.0f;
static volatile float altitudeWorking = 0.0f;
static volatile float groundDistanceWorking = 0.0f;
static float maxAltitudeChangeRate = .8f;
static volatile float invGroundPressure = 0.0f;   // reciprocal of baseline pressure at startup
static volatile float invGroundPressureWorking = 0.0f;   // reciprocal of baseline pressure at startup
...
static float maxYaw = 130.0f;     // Max yaw rate in deg/sec (default 160.0)
static float maxMotor = 1.0f; // this is to tame the beast
...
void setup() {
  ...
  setupGPS();
  setupWiFi();  // At first power on, a WiFi hotspot is set up for talking to the drone. (SSID _Rawpter, Password 12345678, IP 192.168.2.4)
  setupPressureSensor();
  setupIMU();
...
void loopDrone(void *pvParameters) {
  while (true)
  {
    ...
    taskYIELD();                                             // Just a means to allow for other tasks to take their turn.
...
{ // This makes sure the pressure sensing task "bmpTaskHandle", is not writing to our variables so we don't corrupt or data.
  ...
  if (ulTaskNotifyTake(pdTRUE, 0) > 0) {
    altitude = altitudeWorking;
    rateFPS = rateFPSWorking;
    groundDistance = groundDistanceWorking;
  }
}
void beginAltitudeTask()
{
    // Start the BMP581 task on its own core. It is the limiting routine.
...
inline void setAltitudeRate()
{
    ...
    if (filled) {
        ...
        buf[i] = altitudeWorking;           // insert newest at age N-1
        S_y += buf[i];
        S_ty += (N - 1) * buf[i];
        i = (i + 1) % N;
    } else {
        buf[i] = altitudeWorking;
        S_y += buf[i];
    ...
    rateFPSWorking = slope_per_index / dt;
}
...
inline void getAltitude()
{
  ...
  if (ulTaskNotifyTake(pdTRUE, 0) > 0) invGroundPressureWorking=0.0f;
  setAltitude();
  setAltitudeRate();
  if (flying && altitudeWorking > highestAltitude) highestAltitude = altitudeWorking;
  if (altitudeWorking>=2.0f && !flying) {
    if (++flyingCounter>4) // make sure it is not a blip
    {
      flying=true; flyingCounter=0;
    }    
  }
  else flyingCounter = 0;
  xTaskNotifyGive( loopDroneHandle );
}
...
void setAltitude()
{
    ...
    if (err == BMP5_OK)
    {
        if (invGroundPressureWorking == 0.0f) {
            // Capture baseline pressure at ground zero
            invGroundPressureWorking = 1.0f / data.pressure;
            startAltitude = 0.0f;   // define ground zero
        }

        // Ratio of current pressure to baseline
        float ratio = data.pressure * invGroundPressureWorking;

        // Compute relative altitude in feet
        altitudeWorking = (1.0f - expf(exponent * logf(ratio))) * scaleFeet;
    }
    else
    {
        altitudeWorking = startAltitude;   // fallback if sensor read fails
    }

    // Distance above ground zero
    groundDistanceWorking = altitudeWorking - startAltitude;
}
...
void halt()
{  
  while (true) delay(100); // lock forever
}
...
void printGPS() {
  if (tick_time - print_counter > 10000) {
    ...
    if (myGNSS.getInvalidLlh() == false) {
      Serial.print("Lat: "); Serial.println(myGNSS.getLatitude() / 1e7, 7);
      Serial.print("Lon: "); Serial.println(myGNSS.getLongitude() / 1e7, 7);
      Serial.print("Alt: "); Serial.println(myGNSS.getAltitude() / 1000.0, 2);  // meters
    }
  }
}
...
inline void controlMixer() {
  ...
  // Quad mixing
  m1_command_scaled = (maxMotor * throttle_corrected) - pitch_PID + roll_PID + yaw_PID;  // Front left
  m2_command_scaled = (maxMotor * throttle_corrected) - pitch_PID - roll_PID - yaw_PID;  // Front right
  m3_command_scaled = (maxMotor * throttle_corrected) + pitch_PID - roll_PID + yaw_PID;  // Back right
  m4_command_scaled = (maxMotor * throttle_corrected) + pitch_PID + roll_PID - yaw_PID;  // Back left
  ...
  //if (!flying) ConditionCommands();  // Smooth ramping of motor speeds when launching and landing to prevent hard hits.
}
...
void waitForRadio()
{
  ...
  while ( (PWM_throttle_prev < 1400 || PWM_throttle_prev > 1600 || PWM_ThrottleCutSwitch > 1500) && !EASYCHAIR) { //Throttle Cut switch pulled down (flymode) is 2000. Up is 1000 (cut)
    ...
    tick(); getIMUdata(); tock(); //This will warm up the Madgwick while we wait for them to turn on the radio.
    delay(10);
  }
...
void setupBatteryMonitor() {
  ...
  if (!checker) Serial.println("Failed to set up Buzzer.");
}
...
inline void loopBuzzer() {  // this monitors the battery.  the lower it gets, the faster it beeps.
  // disabling for now until we get the wiring correct in the next version
  static unsigned long buzzer_spacing = 22000;
  ...
  if (myTime > next_voltage_check) {
    next_voltage_check = myTime + 22000;  // checkvoltage once every 10 seconds versus every loop.
    buzzer_spacing = 22000;
    /*
...
void setupIMU() {
  ...
  writeRegister(0x4E, 0x1F); delay(10);  // Turn the accelerator and gyro in Low Noise mode and power the temperature sensor.
  writeRegister(0x13, 0x05); delay(10);  // <2ns SPI Slew Rate
  writeRegister(0x16, 0x00); delay(10);  // Bypass FIFO (don't queue readings)
  writeRegister(0x4F, 0x05); delay(10);  // Gyro 2000 dps at 2kHz updates downsampled from 32kHz
  writeRegister(0x50, 0x05); delay(10);  // Accel 32g and 2Khz updates
  writeRegister(0x65, 0x08); delay(10);  // Don't bother interupting after reseet. Interrupt on 1 when data is available.
  writeRegister(0x76, 0x01); delay(10);  // Work with Bank 1
  writeRegister(0x03, 0x00); delay(10);  // Bank 1 register: Use every axis measurement including Z (default has it off)
  writeRegister(0x7A, 0x02); delay(10);  // Bank 1 register: 4 wire SPI mode
  writeRegister(0x76, 0x0); delay(10);   // Work with Bank 0
...
inline void MadgwickInit() {
    ...
    while (gpio_get_level((gpio_num_t)IMU_INT_PIN)) delay(10);  // Wait for it to pull low to indicate data ready
    ...
    if (norm == 0.0f) return; // invalid accel
    ...
    pitch_IMU = pitch * 57.29577951f;
    yaw_IMU   = yaw   * 57.29577951f;
}

inline void getIMUdata() {
  // Fast GPIO read (ESP32 example)
  if (gpio_get_level((gpio_num_t)IMU_INT_PIN)) return;  // active LOW
  ...
  madDeltaTime = (currentMicros - lastMadgwickUpdateMicros) * USEC_TO_SEC;
...
inline void Madgwick6DOF(float gx, float gy, float gz, float ax, float ay, float az) {
  ...
  pitch_IMU = -asinf(twoq0q2 - twoq1q3) * RAD2DEG;
  yaw_IMU   = atan2f(twoq0q3 + twoq1q2, 1.0f - 2.0f*(q2q2 + q3q3)) * RAD2DEG;
}
...
void calculateIMUError() {
  ...
  int c = 0;
  while (c < 30000) {
    while (digitalRead(IMU_INT_PIN) == HIGH) delay(1);
    ...
    {
    // -------- OUTLIER REJECTION --------
      if (fabs(rawAccX - fAccX) < OUTLIER_THRESHOLD_ACC)  fAccX = fAccX + ALPHA * (rawAccX - fAccX);
      if (fabs(rawAccY - fAccY) < OUTLIER_THRESHOLD_ACC)  fAccY = fAccY + ALPHA * (rawAccY - fAccY);
      if (fabs(rawAccZ - fAccZ) < OUTLIER_THRESHOLD_ACC)  fAccZ = fAccZ + ALPHA * (rawAccZ - fAccZ);

      if (fabs(rawGyroX - fGyroX) < OUTLIER_THRESHOLD_GYRO) fGyroX = fGyroX + ALPHA * (rawGyroX - fGyroX);
      if (fabs(rawGyroY - fGyroY) < OUTLIER_THRESHOLD_GYRO) fGyroY = fGyroY + ALPHA * (rawGyroY - fGyroY);
      if (fabs(rawGyroZ - fGyroZ) < OUTLIER_THRESHOLD_GYRO) fGyroZ = fGyroZ + ALPHA * (rawGyroZ - fGyroZ);
    }
    ...
    if (c % 500 == 0) Serial.println(String(30000 - c));
  }
  ...
  Serial.print("static float AccErrorX = "); Serial.print(AccErrorX); Serial.println(";");
  Serial.print("static float AccErrorY = "); Serial.print(AccErrorY);Serial.println(";");
  Serial.print("static float AccErrorZ = "); Serial.print(AccErrorZ);Serial.println(";");

  Serial.print("static float GyroErrorX = "); Serial.print(GyroErrorX);Serial.println(";");
  Serial.print("static float GyroErrorY = "); Serial.print(GyroErrorY);Serial.println(";");
  Serial.print("static float GyroErrorZ = "); Serial.print(GyroErrorZ);Serial.println(";");
...
void AngleLoopCalcs() {
    ...
    if (++PIDCounter<(INNER_LOOP_FREQUENCY/PID_FREQ_HZ)) return;
    PIDCounter = 0;

    dt = micros() - lastTimeMicros; lastTimeMicros = micros();
...
void PIDControlCalcs() {
    ...
    if (PWM_throttle < 1520) // Reset the control if on the ground.
    {
    ...
    float derivative_roll = (rateErrorRoll - prevRateErrorRoll) / madDeltaTime;
    ...
    float derivative_pitch = (rateErrorPitch - prevRateErrorPitch) / madDeltaTime;
...
inline void getRadioStickValues() {
  ...
  else
  {
    if (flying)
    {
      if (PWM_throttle>highestThrottlePWM) highestThrottlePWM = PWM_throttle;
      else if (PWM_throttle<lowestThrottlePWM) lowestThrottlePWM = PWM_throttle;
    }
    // Ceiling override
    if (groundDistance > ceiling) {
      if (++throttleOverrideCounter >= CEILING_PERIOD) {
        throttleOverrideCounter = 0;  // reset counter
        PWM_throttle = failsafeThrottlePWM;
        ceilingOverride = true;
      }
    }
    // Descent override
    else if (rateFPS < -maxAltitudeChangeRate && groundDistance > 3.0f) {
      if (++throttleOverrideCounter >= CEILING_PERIOD) {
        throttleOverrideCounter = 0;  // reset counter
        PWM_throttle = clamp(PWM_throttle_prev + 1, 1500, 2000);
      }
    }
    // Ascent override
    else if (rateFPS > maxAltitudeChangeRate && groundDistance > 5.0f) {
      if (++throttleOverrideCounter >= ASCENT_PERIOD) {
        throttleOverrideCounter = 0;  // reset counter
        PWM_throttle = clamp(PWM_throttle_prev - 1, 1000, 2000);
      }
    }
    // Normal smoothing
    else {
  ...
  // Bottom limit
  if (PWM_throttle < 1500) PWM_throttle = 1500;
...
inline void motorPipeline()
{
  ...
  if (ESCWriteCounter<(INNER_LOOP_FREQUENCY/MOTOR_FREQ_HZ)) return;
...
inline void calibrateESCs() {
  ...
  Serial.print("Radio Cut PWM: "); Serial.println(raw);
  
  if (getRadioPWM(throttleCutSwitchPin,2000)>1500) return; //throttle cut switch up, then run the calibration. If off or down, skip it.
...
*/
  ...
  if (throttle_is_cut)
  {
    ...
    if (PWM_ThrottleCutSwitch > 1500) { // switch is in the down position which means enable flight
    // reset (uncut throttle) only if throttle is down to prevent a jolting suprise
      if (PWM_throttle < 1520 && ++throttleNotCutCounter > 10) { // The radio is ready for flight and confirmed not to be just a blip.
        throttle_is_cut = false;
        throttleNotCutCounter = 0;
        throttleCutCounter = 0;
        flying = false;
        xTaskNotifyGive(bmpTaskHandle); // Notify that we want the altitude to reset to current ground level
  ...
  if (!flying||altitude<2) // this is tip over protection at take-off or if it is having a hard landing.
  {
...
void tock() {
  ...
  if (resetTimers) resetAllTiming(); // Resets the counters that keep the beat for outer loops
}
...
void printJSON() {
  if (tick_time - print_counter > 10000) {  // Don't go too fast or it slows down the main loop. this is a third of a second.
    print_counter = micros();
    Serial.print(F("{\"roll\": "));
...
void loopWiFi() {
  ...
  // --- Your form submission ---
    ...
    Serial.println("Values set, generating the page...");
    GenerateDefaultPage(client);
    Serial.println("Page generated.");
  }
...
void setValuesFromUserForm(String req) {
  ...
  while (start < query.length()) {
    ...
    // Assign values
    if (key == "maxMotor") maxMotor = value.toFloat();
    else if (key == "stick_dampener") stick_dampener = value.toFloat();
    else if (key == "i_limit_rate") i_limit_rate = value.toFloat();
    else if (key == "i_limit") i_limit = value.toFloat();
    else if (key == "Accel_filter") Accel_filter = value.toFloat();
    else if (key == "Gyro_filter") Gyro_filter = value.toFloat();
    else if (key == "B_madgwick") B_madgwick = value.toFloat();
    else if (key == "kp_roll_angle") Kp_roll_angle = value.toFloat();
    else if (key == "ki_roll_angle") Ki_roll_angle = value.toFloat();
    else if (key == "kd_roll_angle") Kd_roll_angle = value.toFloat();
    else if (key == "kp_pitch_angle") Kp_pitch_angle = value.toFloat();
    else if (key == "ki_pitch_angle") Ki_pitch_angle = value.toFloat();
    else if (key == "kd_pitch_angle") Kd_pitch_angle = value.toFloat();
    else if (key == "kp_roll_rate") Kp_roll_rate = value.toFloat();
    else if (key == "ki_roll_rate") Ki_roll_rate = value.toFloat();
    else if (key == "kd_roll_rate") Kd_roll_rate = value.toFloat();
    else if (key == "kp_pitch_rate") Kp_pitch_rate = value.toFloat();
    else if (key == "ki_pitch_rate") Ki_pitch_rate = value.toFloat();
    else if (key == "kd_pitch_rate") Kd_pitch_rate = value.toFloat();
    else if (key == "kp_yaw_rate") Kp_yaw_rate = value.toFloat();
    else if (key == "ki_yaw_rate") Ki_yaw_rate = value.toFloat();
    else if (key == "kd_yaw_rate") Kd_yaw_rate = value.toFloat();
    else if (key == "roll_maxRate") rollLimits.maxRate = value.toFloat();
    else if (key == "roll_maxAccel") rollLimits.maxAccel = value.toFloat();
    else if (key == "roll_maxJerk") rollLimits.maxJerk = value.toFloat();
    else if (key == "pitch_maxRate") pitchLimits.maxRate = value.toFloat();
    else if (key == "pitch_maxAccel") pitchLimits.maxAccel = value.toFloat();
    else if (key == "pitch_maxJerk") pitchLimits.maxJerk = value.toFloat();

    else if (key == "failsafeThrottlePWM") failsafeThrottlePWM = value.toInt();
    else if (key == "trimPitch") trimPitch = value.toFloat();
    else if (key == "trimYaw") trimYaw = value.toFloat();
    else if (key == "trimRoll") trimRoll = value.toFloat();
    else if (key == "action") {
      if (value == "SAVE TO STORAGE") saveParameters();
    }
...
void GenerateDefaultPage(WiFiClient &client) {
  ...
  String body;
  body.reserve(8192); // optional: pre-allocate to reduce reallocs
  ...
  body += "input[type=text], input[type=number] {"
          "font-size:20px; padding:8px; width:180px; box-sizing:border-box;"
  ...
  body += "<b>Snapshot:</b><br>Desired Roll=" + String(roll_des) +
          "&#176; IMU Roll=" + String(roll_IMU) + "&#176;<br>";

  body += "Desired Pitch=" + String(pitch_des) +
          "&#176; IMU Pitch=" + String(pitch_IMU) + "&#176;<br>";

  body += "Loop Time=" + String(int(round( (deltaTime) / 1000000))) +
          " Throttle PWM=" + String(PWM_throttle) + "<br>";

  body += "Battery=" + String(calced_voltage, 1) + "V (" +
          String(batteryVoltage) + ")<br>";
  body += "Highest Altitude=" + String(highestAltitude) + "<br>";
  body += "Highest Throttle=" + String(highestThrottlePWM) + "<br>";
  body += "Lowest Throttle=" + String(lowestThrottlePWM) + "<br>";
  body += "Battery=" + String(calced_voltage, 1) + "V (" +
          String(batteryVoltage) + ")<br>";
  body += "Altitude=" + String(altitude) + " ft<br><hr>";

  if (calced_voltage < 13) {
    body += "<br><div class='alert alert-danger'>DANGER: BATTERY CRITICAL!</div><br>";
  } else if (calced_voltage < 14) {
    body += "<br><div class='alert alert-warning'>Warning: BATTERY LOW!</div><br>";
  }

  // Parameters table
  body += "<table class=table><thead class=thead-dark>"
          "<th></th><th>Kp</th><th>Ki</th><th>Kd</th></thead>";

  // Roll row
  body += "<tr><td>Roll Angle:</td>"
          "<td><input name=kp_roll_angle style='width:70px;' type=number step=0.01 value='" + String(Kp_roll_angle) + "'></td>"
          "<td><input name=ki_roll_angle style='width:70px;' type=number step=.001 value='" + String(Ki_roll_angle,3) + "'></td>"
          "<td><input name=kd_roll_angle type=number step=.001 style='width:70px;' value='" + String(Kd_roll_angle,3) + "'></td></tr>";

  body += "<tr><td>Roll Rate:</td>"
          "<td><input name=kp_roll_rate style='width:70px;' type=number step=0.01 value='" + String(Kp_roll_rate) + "'></td>"
          "<td><input style='width:70px;' name=ki_roll_rate type=number step=.001 value='" + String(Ki_roll_rate,3) + "'></td>"
          "<td><input name=kd_roll_rate type=number step=.001 style='width:70px;' value='" + String(Kd_roll_rate,3) + "'></td></tr>";

  // Pitch row
  body += "<tr><td>Pitch Angle:</td>"
          "<td><input name=kp_pitch_angle style='width:70px;' type=number step=0.01 value='" + String(Kp_pitch_angle) + "'></td>"
          "<td><input style='width:70px;' name=ki_pitch_angle type=number step=.001 value='" + String(Ki_pitch_angle,3) + "'></td>"
          "<td><input name=kd_pitch_angle type=number step=.001 style='width:70px;' value='" + String(Kd_pitch_angle,3) + "'></td></tr>";

  body += "<tr><td>Pitch Rate:</td>"
          "<td><input name=kp_pitch_rate style='width:70px;' type=number step=0.01 value='" + String(Kp_pitch_rate) + "'></td>"
          "<td><input style='width:70px;' name=ki_pitch_rate type=number step=.001 value='" + String(Ki_pitch_rate,3) + "'></td>"
          "<td><input name=kd_pitch_rate type=number step=.001 style='width:70px;' value='" + String(Kd_pitch_rate,3) + "'></td></tr>";

  // Yaw row
  body += "<tr><td>Yaw Rate:</td>"
          "<td><input name=kp_yaw_rate type=number step=0.01 style='width:70px;' value='" + String(Kp_yaw_rate) + "'></td>"
          "<td><input style='width:70px;' type=number step=.001 name=ki_yaw_rate value='" + String(Ki_yaw_rate,3) + "'></td>"
          "<td><input type=number step=.001 name=kd_yaw_rate style='width:70px;' value='" + String(Kd_yaw_rate,3) + "'></td></tr>";

  // Integral limit row
  body += "<tr><td>Integral Accumulation (3 default):</td>"
          "<td><input type=number step=.001 name=i_limit style='width:90px;' value='" + String(i_limit) + "'></td></tr>";
  body += "<tr><td>Rate Integral Accumulation:</td>"

          "<td><input type=number step=.001 name=i_limit_rate style='width:90px;' value='" + String(i_limit_rate) + "'></td></tr>";

  body += "<tr><td>B_Madgwick (0.06 default):</td>"
          "<td><input type=number step=.001 name=B_madgwick style='width:90px;' value='" + String(B_madgwick) + "'></td></tr>";

  body += "</table><br>";
  
  body += "<table class=table><thead class=thead-dark>"
          "<th></th><th>Max Rate</th><th>Max Accel</th><th>Max Jerk</th></thead>";
  body += "<tr><td>Roll:</td>"
          "<td><input name=roll_maxRate style='width:70px;' step=1 type=number value='" + String(rollLimits.maxRate) + "'></td>"
          "<td><input style='width:70px;' name=roll_maxAccel step=1 type=number value='" + String(rollLimits.maxAccel) + "'></td>"
          "<td><input name=roll_maxJerk step=1 type=number style='width:90px;' value='" + String(rollLimits.maxJerk) + "'></td></tr>";

  // Pitch row
  body += "<tr><td>Pitch:</td>"
          "<td><input name=pitch_maxRate style='width:70px;' step=1 type=number value='" + String(pitchLimits.maxRate) + "'></td>"
          "<td><input style='width:70px;' name=pitch_maxAccel step=1 type=number value='" + String(pitchLimits.maxAccel) + "'></td>"
          "<td><input name=pitch_maxJerk step=1 type=number style='width:90px;' value='" + String(pitchLimits.maxJerk) + "'></td></tr>";
  ...
  body += "<tr><td>Failsafe Throttle (1500 to 2000):</td>"
          "<td><input type=number name=failsafeThrottlePWM style='width:80px;' value='" + String(failsafeThrottlePWM) + "'></td></tr>";

  body += "<tr><td>Trim - Pitch (-500 to 500):</td>"
          "<td><input type=number name=trimPitch style='width:80px;' value='" + String(trimPitch) + "'></td></tr>";

  body += "<tr><td>Trim - Roll (-500 to 500):</td>"
          "<td><input type=number name=trimRoll style='width:80px;' value='" + String(trimRoll) + "'></td></tr>";

  body += "<tr><td>Trim - Yaw (-500 to 500):</td>"
          "<td><input type=number name=trimYaw style='width:80px;' value='" + String(trimYaw) + "'></td></tr>";

  body += "<tr><td>Max Motor Speed (0.0-1.0):</td>"
          "<td><input type=number step=0.01 name=maxMotor style='width:80px;' value='" + String(maxMotor,2) + "'></td></tr>";

  body += "<tr><td>Stick Dampening (0.01-1.0):<br>0.1=slow/steady, 1.0=noisy/fast</td>"
          "<td><input type=number step=0.001 name=stick_dampener style='width:80px;' value='" + String(stick_dampener, 3) + "'></td></tr>";

  body += "<tr><td>Accel Dampening (0.1-1.0):<br>0.1=slow/steady, 1.0=noisy/fast</td>"
          "<td><input type=number step=0.01 name=Accel_filter style='width:80px;' value='" + String(Accel_filter) + "'></td></tr>";

  body += "<tr><td>Gyro Dampening (0.1-1.0):<br>0.1=slow/steady, 1.0=noisy/fast</td>"
          "<td><input type=number step=0.01 name=Gyro_filter style='width:80px;' value='" + String(Gyro_filter) + "'></td></tr>";

  body += "</table>";

  // Buttons
  body += "<br><input style='margin-top:8px; padding:6px 12px; background-color:#0d6efd; color:#fff; border:1px solid #0d6efd; border-radius:4px; width:100%; cursor:pointer; font-size:20px;' type='submit' name='action' value='SUBMIT' />";
  body += "<br><br><br><input style='margin-top:8px; padding:6px 12px; background-color:#ff0000; color:#fff; border:1px solid #0d6efd; border-radius:4px; width:100%; cursor:pointer; font-size:20px;' type='submit' name='action' value='SAVE TO STORAGE' />";

  body += "</div></form>";

  // Tuning procedure and tip
...
void saveParameters() {
  ...
  prefs.putFloat("maxMotor", maxMotor);
  ...
  prefs.putFloat("Kd_roll_angle", Kd_roll_angle);
  ...
  prefs.putFloat("Kd_pitch_angle", Kd_pitch_angle);
  prefs.putFloat("roll_maxRate", rollLimits.maxRate);
  prefs.putFloat("roll_maxAccel", rollLimits.maxAccel);
  prefs.putFloat("roll_maxJerk", rollLimits.maxJerk);
  prefs.putFloat("pitch_maxRate", pitchLimits.maxRate);
  prefs.putFloat("pitch_maxAccel", pitchLimits.maxAccel);
  prefs.putFloat("pitch_maxJerk", pitchLimits.maxJerk);
...
void loadParameters() {
  ...
  maxMotor        = prefs.getFloat("maxMotor", maxMotor);
  ...
  Kd_roll_angle   = prefs.getFloat("Kd_roll_angle", Kd_roll_angle);
  ...
  Kd_pitch_angle  = prefs.getFloat("Kd_pitch_angle", Kd_pitch_angle);

  rollLimits.maxRate   = prefs.getFloat("roll_maxRate", rollLimits.maxRate);
  rollLimits.maxAccel  = prefs.getFloat("roll_maxAccel", rollLimits.maxAccel);
  rollLimits.maxJerk   = prefs.getFloat("roll_maxJerk", rollLimits.maxJerk);
  pitchLimits.maxRate  = prefs.getFloat("pitch_maxRate", pitchLimits.maxRate);
  pitchLimits.maxAccel = prefs.getFloat("pitch_maxAccel", pitchLimits.maxAccel);
  pitchLimits.maxJerk  = prefs.getFloat("pitch_maxJerk", pitchLimits.maxJerk);
